...
<div class="d-flex justify-content-between align-items-center mb-4">
    <h2 class="section-title">
        <i class="fas fa-edit me-2"></i>Editar Pago #{{ $payment->id }}
    </h2>
                ...
                <div class="col-md-4">
                    <div class="mb-3">
                        ...
                        <select class="form-select @error('currency') is-invalid @enderror" id="currency" name="currency" required>
                            ...
                            <option value="cop" {{ (old('currency') ?? $payment->currency) == 'cop' ? 'selected' : '' }}>COP</option>
                ...
                <div class="col-md-4">
                    <div class="mb-3">
                        ...
                        <select class="form-select @error('payment_method') is-invalid @enderror" id="payment_method" name="payment_method" required>
                            ...
                            <option value="card" {{ (old('payment_method') ?? $payment->payment_method) == 'card' ? 'selected' : '' }}>Tarjeta</option>
                            <option value="bank_transfer" {{ (old('payment_method') ?? $payment->payment_method) == 'bank_transfer' ? 'selected' : '' }}>Transferencia</option>
            ...
            <div class="row">
                <div class="col-md-6">
                    <div class="mb-3">
                        ...
                        <select class="form-select @error('status') is-invalid @enderror" id="status" name="status" required>
                            ...
                            <option value="refunded" {{ (old('status') ?? $payment->status) == 'refunded' ? 'selected' : '' }}>Reembolsado</option>
                        </select>
                        @error('status')
                            <div class="invalid-feedback">{{ $message }}</div>
                        @enderror
                ...
                <div class="col-md-6">
                    <div class="mb-3">
                        ...
                        <input type="text" class="form-control" value="{{ $payment->created_at->format('d/m/Y H:i') }}" readonly>
                    </div>
                </div>
            </div>
...
@push('scripts')
<script>
// Auto-fill amount and currency when course is selected (only if amount is empty)
document.getElementById('course_id').addEventListener('change', function() {
    const selectedOption = this.options[this.selectedIndex];
    const amountField = document.getElementById('amount');
    
    if (selectedOption.value && !amountField.value) {
